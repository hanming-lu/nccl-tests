...
using fp16_t = __half;
#define fp162float(X) __half2float(X)
...
__global__ void customized_all_gather(fp16_t *embedding, fp16_t *block_global_data, int num_elements, Coordinator* coordinator) {
    int mype = nvshmem_my_pe();
    int npes = nvshmem_n_pes();
    for (int iter = 0; iter < ITER; ++iter) {
        fp16_t val = __float2half(static_cast<float>(mype + 1));

        int32_t elements_per_pe = num_elements / npes;
        int32_t elements_per_block = elements_per_pe / NUM_BLOCKS;
        for (int i = threadIdx.x; i < elements_per_block; i += NUM_THREADS_PER_BLOCK) {
            int32_t idx_with_offset = blockIdx.x * elements_per_block + i;
        ...
        nvshmemx_put16_block(embedding + mype * elements_per_pe, block_global_data, elements_per_pe, peer1);
        nvshmemx_put16_block(embedding + mype * elements_per_pe, block_global_data, elements_per_pe, peer2);
        nvshmemx_put16_block(embedding + mype * elements_per_pe, block_global_data, elements_per_pe, peer3);
        if (blockIdx.x == 0 && threadIdx.x == 0) {
            for (int i = mype * elements_per_pe; i < ((mype + 1) * elements_per_pe); i++) {
                embedding[i] = val;
            }
            nvshmem_barrier_all();
        }
...
int main(int argc, char *argv[]) {
    ...
    if (mype == 0) {
        ...
        printf("NUM_BLOCKS: %d, NUM_THREADS_PER_BLOCK: %d, bs: %d\n", NUM_BLOCKS, NUM_THREADS_PER_BLOCK, bs);
    }

    int num_elements = 8192 * bs;
    int elements_per_pe = num_elements / npes;

    fp16_t *embedding_all_gather = (fp16_t *)nvshmem_calloc(num_elements, sizeof(fp16_t));
    if (embedding_all_gather == NULL) {
        fprintf(stderr, "nvshmem_calloc for all gather failed\n");
        exit(EXIT_FAILURE);
    }

    fp16_t *block_global_data;
    cudaMalloc((void **)&block_global_data, elements_per_pe * sizeof(fp16_t));
    ...
    fp16_t *host_embedding_all_gather = (fp16_t *)malloc(num_elements * sizeof(fp16_t));
    cudaMemcpy(host_embedding_all_gather, embedding_all_gather, num_elements * sizeof(fp16_t), cudaMemcpyDeviceToHost);

    // if (mype == 3) {
    //     printf("Results -- All Gather Embedding from GPU: %d\n", mype);
    //     // Sample prints
    //     for (int i = 0; i < num_elements; i += 256) {
    //         printf("%f ", fp162float(host_embedding_all_gather[i]));
    //     }
    //     printf("\n");
    // }
