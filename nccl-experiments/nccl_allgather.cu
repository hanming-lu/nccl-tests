...
#define ITER 1000
// For h100 - it's 128; for V100 - it's 32.
...
__global__ void multiplyByFactor(float* data, int size, int dev_idx) {
  // int idx = blockIdx.x * blockDim.x + threadIdx.x;
  for (int idx = blockIdx.x * blockDim.x + threadIdx.x; idx < size; idx += NUM_THREADS_PER_BLOCK*NUM_BLOCKS) {
    data[idx] = static_cast<float>(0.01 * dev_idx);
  }
}
...
void runWithoutCudaGraph(float** buff, cudaStream_t* s, ncclComm_t* comms, int nDev, int size) {
  ...
  for (int iter = 0; iter < ITER; ++iter) {
    ...
    for (int i = 0; i < nDev; ++i) {
      ...
      multiplyByFactor<<<blocksPerGrid, threadsPerBlock>>>(buff[i]+ i * (size / nDev), size / nDev, i);
    }
    ...
    for (int i = 0; i < nDev; ++i)
      NCCLCHECK(ncclAllGather((const void*)(buff[i] + i * (size / nDev)), (void*)buff[i], size / nDev, ncclFloat,
          comms[i], s[i]));
...
void runWithCudaGraph(float** buff, cudaStream_t* s, ncclComm_t* comms, int nDev, int size) {
  ...
  for (int iter = 0; iter < ITER; ++iter) {
    ...
    for (int i = 0; i < nDev; ++i) {
      CUDACHECK(cudaSetDevice(i));
      multiplyByFactor<<<blocksPerGrid, threadsPerBlock, 0, s[i]>>>(buff[i]+ i * (size / nDev), size / nDev, i);
    }
...
int main(int argc, char* argv[])
{
  ...
  // For h100 - 8 GPUs; for V100 - 4 GPUs.
  int nDev = 8;
  int size = bs * 8192;
